...
const MyPosts = () => {
  ...
  useEffect(() => {
    fetch(`https://stark-springs-15017.herokuapp.com/myPost/${user?.email}`)
      .then((res) => res.json())
  ...
  const handleMyPostDelete = (id) => {
    window.confirm("Are you sure you wish to delete this item?") &&
      axios
        ...
        .then((res) => {
          res.data.deletedCount && alert("Delete Success");
          fetch(
            `https://stark-springs-15017.herokuapp.com/myPost/${user?.email}`
          )
            .then((res) => res.json())
            .then((data) => setTravels(data));
        });
    ...
    <div>
      {currentPosts?.length === 0 ? (
        <h4 className=" text-pink-800 text-center">
          No posts found for you, please post first
        </h4>
      ) : (
        <h1 className=" text-pink-600 text-center">
          My Post {currentPosts?.length}{" "}
        </h1>
      )}

      {!currentPosts?.length && (
        <img src="https://cdn.wallpapersafari.com/99/54/lsZuYi.jpg" alt="" />
      )}
        ...
        <div className="max-w-6xl mx-auto">
          <div className=" flex justify-center flex-wrap">
            {currentPosts?.reverse()?.map((travel) => (
              <div
                key={travel?._id}
                className="max-w-sm w-full sm:w-1/2 lg:w-1/3 px-2 py-6 cursor-pointer"
              >
                <div className="bg-white shadow-xl rounded-lg overflow-hidden">
                  ...
                  <div className=" text-center">
                    <button className=" text-amber-400 text-3xl btn">
                      Pending
                    </button>
                  </div>
                  <div className="flex p-4 border-t border-gray-300 text-gray-700 justify-between">
                    ...
                    <Button onClick={() => handleMyPostDelete(travel?._id)}>
                      Delete
                    </Button>
