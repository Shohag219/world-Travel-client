...
const ManageAllPosts = () => {
  ...
  const handleApprove = (id) => {
    fetch(`https://stark-springs-15017.herokuapp.com/approve/${id}`, {
      method: "PUT",
      headers: {
    ...
    <section className="text-gray-600">
      {currentPosts?.length === 0 ? (
        <h4 className=" text-pink-800 text-center">
          No posts found {currentPosts?.length}
        </h4>
      ) : (
        <h1 className=" text-pink-600 text-center">
          Post Found: {currentPosts?.length}{" "}
        </h1>
      )}
      {!currentPosts?.length && (
            ...
            >
              <div className="bg-white shadow-xl rounded-lg overflow-hidden">
                ...
                <div className="flex p-4 border-t border-gray-300 text-gray-700 justify-between">
                  ...
                  <Button
                    onClick={() => handleApprove(travel?._id)}
                    color="secondary"
                  >
                    Approve
                  </Button>
