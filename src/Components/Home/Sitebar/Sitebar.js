...
const Sitebar = () => {
  ...
  return (
    <div>
      <h1 className=" text-3xl  text-center pt-4  font-bold border-b mb-3">
        Top travel{" "}
      </h1>
      <div className=" flex justify-center  overflow-y-auto max-h-screen scroll-b">
        <div>
          {topRating?.reverse()?.map((topBlog) => (
            <div
              key={topBlog?._id}
              className="max-w-sm w-full px-2 py-6  cursor-pointer"
            >
              <div className="bg-white shadow-xl rounded-lg overflow-hidden">
                ...
                <div className="flex p-4 border-t border-gray-300 text-gray-700 justify-between">
                  <Link to={`travelsDetails/${topBlog?._id}`}>
                    <Button>Details</Button>
                  </Link>
                  <Rating value={topBlog?.rating} precision={0.5} readOnly />
                </div>
              </div>
            </div>
          ))}
